...
generate_socks_subscription() {
    ...
    # 生成 SOCKS5 订阅链接 (格式: socks5://username:password@host:port)
    local sub_link="socks5://${user}:${pass}@${ip}:${port}"
    ...
    # 将订阅链接保存到文件
    echo $sub_link > /etc/xrayL/socks_subscription.txt
    echo "订阅链接已保存到: /etc/xrayL/socks_subscription.txt"
}
...
config_xray() {
    ...
    if [ "$config_type" == "socks" ]; then
        ...
        # 生成订阅链接
        generate_socks_subscription ${IP_ADDRESSES[0]} $START_PORT $SOCKS_USERNAME $SOCKS_PASSWORD
    elif [ "$config_type" == "vmess" ]; then
        echo "UUID:$UUID"
        echo "ws路径:$WS_PATH"
    fi
